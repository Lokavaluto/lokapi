...
You can toggle the "favorite" state of a given contact with the
...
let newContact = await lokAPI.toggleFavorite(recipients[2])
console.log(`${newContact.name} ${newContact.is_favorite ? 'IS' : 'IS NOT'} one of my favorite.`)
#+end_src
