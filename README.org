...
let accounts = await backends[0].getAccounts()

let balance = await accounts.getBalance()
let symbol= await accounts.getSymbol()

console.log(`balance in first account: ${balance} ${symbol}`)
#+end_src

- =lokAPI.getBackends()= is the list of backends connections that are
