...
*** list backends instances and minimum credit amount
...
=Backend= objects offers =minCreditAmount= that allows you to know
what would be the minimum accepted amount for credit requests.

#+begin_src typescript
let backends = await lokAPI.getBackends()

for (const backend of backends) {
    console.log(`  ${backend.internalId}: min: ${backend.minCreditAmount}`)
}
#+end_src
